...
[myiam-cli](https://github.com/myiam-io/myiam-cli)용 AI 코딩 에이전트 플러그인. myiam-cli는 MyIAM 서비스(OAuth2 클라이언트, 로그인 타입, 정책, 약관, 사용자 필드, UI/브랜딩, API 설정 등)를 관리하는 관리자 도구이며, 이 플러그인은 Claude Code(`/myiam`)와 Codex CLI(`@myiam`) 스킬을 통해 전체 명령어 레퍼런스를 제공한다.
...
`skills/myiam/SKILL.md`는 `../myiam-cli/skills/myiam-cli/SKILL.md`의 사본이다 — **원본을 그쪽에서 먼저 수정한 뒤 이 저장소로 복사**한다 (`cp ../myiam-cli/skills/myiam-cli/SKILL.md skills/myiam/SKILL.md`). 단, 원본의 frontmatter는 `name: myiam-cli`이고 이 저장소는 명령어를 `/myiam`으로 줄이기 위해 `name: myiam`으로 바꿔둔 상태다 — **복사한 뒤 frontmatter의 `name` 필드만 다시 `myiam`으로 고칠 것** (본문 내용은 그대로 덮어써도 됨). 이 저장소에서 본문 내용을 직접 고치지 않는다 — 두 파일이 갈라지면 다음 동기화 때 덮어써진다. myiam-cli의 실제 명령어와 항상 동기화 상태를 유지해야 한다. 주요 규칙:
